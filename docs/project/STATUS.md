...
- Current plan SHA-256 is `5f18998f11c17b8feef52f90edd9319532a36d525dbea9e9a40538425a28dfa4`; c1-c6 start 20:40/21:10/21:40/22:10/22:40/23:10Z. Predictions remain 3/2/4/1/1/450.
...
- Regenerated bundle SHA-256 is `5a69eb4394f64c1e666aeb624cac3e4e312b3758a9e48f311a8cb0eef610f7dd`, with 462 cases, five replay observations, and no persisted key. Current prefixes begin `dev_recall_m2_compressed_p5f18998f11c1_`. The old unscoped-prefix contents, their untouched state, and absence of accepted manifest references are `OWNER_REPORTED`; independent cloud inventory/read-back is `NOT VERIFIED`.
- Exact-current-tree local verification is the focused compressed suite, 27/27 PASS with direct exit 0. The bounded deterministic core 345/345, platform 259/259, privacy 140/140, offline web 48/48, and the mode-unspecified 345-plus-four-setup-error run are predecessor-tree evidence and were `NOT RERUN` on product commit `2d8bebbe97794865f77f037dea518a39e8f75e38`. Independent product-diff review and Master Judge returned PASS with no product P0/P1 blocker; the evidence/docs Judge required and received these provenance corrections.
...
- F5-lite is implemented with a contract-major transition to `CohortDayManifest 2.0.0`: startup requires a 40-character lowercase-hex `RECALL_SOURCE_COMMIT` equal to preparation bundle provenance and a lowercase `sha256:<64 hex>` `RECALL_IMAGE_DIGEST`; disagreement fails before ledger construction. The bundle commit remains cohort provenance; the image digest names the executing immutable code image.
...
- The L2 Day-N base implementation is committed locally at `367637b12e92eda0c2aa54c8bdc12af3adbfe99d`. It derives the selected date from the actual UTC clock, rejects frozen Day-1 recurrence, uses date-isolated collection prefixes, records actual execution time separately from logical schedule time, reconciles idempotent retries, and emits one strict cohort manifest plus a downstream `DataModeReceipt`.
...
- Current fix-batch verification: focused 36/36, bounded core 299/299, platform 234/234 excluding the known environment-bound token process file, privacy 140/140, and offline web 48/48. Independent code review PASS. A fresh live-Firestore full-suite attempt hung in its authentication/subprocess path and is not claimed as PASS; the earlier base-commit live evidence remains historical, not transferred to this uncommitted successor.
- Claim boundary: the date-isolated Day-N implementation and deterministic local tests are verified. Cloud Run Job/Scheduler/IAM deployment, rebuilt-image identity, workload-identity behavior, actual managed Day-2 execution, billing/cost evidence, and live Day-2 read-back remain `NOT VERIFIED`. Missing-previous-day typed continuation is deferred to Day-3 morning; current Day-3/4 behavior remains fail-closed. Managed admission, cross-day WatchCase continuity, and terminal agent execution are not claimed. No `infra/**` file was changed by L2.
