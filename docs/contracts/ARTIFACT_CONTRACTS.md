...
| `CohortDayManifest` | `3.0.0` | Compressed machine-triggered cycle evidence; strict `2.0.0` and `2.1.0` wires remain read-only legacy contracts |
...
| `CohortDayManifest` | Version `3.0.0`; legacy fields plus `cycle_id`, `cycle_index`, `plan_version`, `plan_sha256`, `cohort_due_date`, `window_start`, `window_end`, `schedule_mode`, `headroom_receipt_id`; post-parse verification binds all cycle rows to the hash-verified `COMPRESSED_PREDICTION_PLAN_V2` artifact | Cohort scheduler | UI cohort panel, operations evidence | Wall-clock substitution, duplicated schedule truth, source/image/plan identity drift, prediction drift, hidden historical failure, unbound c6 admission |
...
| `CohortDayManifest.execution_history[]` | `3.0.0`: monotone `sequence_index`, per-row `source_schema_version`, cycle identity, logical due date, `scheduled_for`, declared window, `trigger_code`, real `executed_at`, counts, status, failure receipt, evidence state, and schedule mode. Day-1 and the owner-reported 2026-08-26 16:00Z failure are exact immutable prefix rows; every compressed row is bound to the hash-locked plan identity, prediction, and window and must appear once in c1-to-current order. Strict `2.0.0`/`2.1.0` parsing remains unchanged. |
| `COMPRESSED_PREDICTION_PLAN_V2.cycles[].predecessor` | Plan schema `2.1.0`: c1 has no predecessor; an external predecessor carries exact plan SHA-256, collection prefix, cycle ID, and manifest artifact ID; a current-plan predecessor carries only the preceding cycle ID and derives immutable identifiers from the current plan. Missing, drifted, or cross-cycle bindings fail closed before current-cycle writes. |
| `COMPRESSED_PREDICTION_PLAN_V2.cycles[].write_path` | `EXTERNAL_IMMUTABLE` prevents replay of an already executed cycle; `SERIAL_VERIFIED` selects the existing verified path; `FIRESTORE_BATCH_V1` is an execution gate and must fail before ledger construction until the reviewed batching path exists. |
