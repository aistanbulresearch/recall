"""Per-request identity-token provider for the IAM-only GPU service.

L1's decision (2026-08-27): the Cloud Run perimeter admits by IAM identity,
audience is not enforced at runtime, and Ollama itself has no auth, so the ID
token is the single gate. The runner is a local human ADC account, therefore
the token is minted by `gcloud auth print-identity-token` as a SUBPROCESS PER
...
def identity_token_header() -> Mapping[str, str]:
    """Mint a fresh ID token and shape it as the Authorization header.
    ...
    try:
        ...
        raise RuntimeError("gcloud_identity_token_timeout") from error
    if completed.returncode != 0:
        ...
        raise RuntimeError(f"gcloud_identity_token_failed: {detail}")
    token = completed.stdout.strip()
    if not token:
        raise RuntimeError("gcloud_identity_token_empty")
