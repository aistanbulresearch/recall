...
| `CohortDayManifest` | `3.2.0` | Adds FULL_AUDIT_V1 aggregate telemetry and case-bound terminal outcomes; strict `2.0.0`, `2.1.0`, `3.0.0`, and `3.1.0` wires remain read-only legacy contracts |
...
| `CohortDayManifest` | Version `3.2.0`; `3.1.0` fields plus `agent_execution_summary` and case-bound `run_outcomes`; post-parse verification binds outcome IDs to ledger artifacts and the hash-verified plan | Cohort scheduler | UI cohort panel, operations evidence | Wall-clock substitution, source/image/plan drift, warm-cache shortcut, aggregate-only agent claims, Agent Engine misattribution |
...
| `CohortDayManifest.execution_history[]` | `3.2.0`: monotone history retaining the exact `3.0.0` c1/c2 rows and adding completion-time ramp rows. `executed_at` is the measured completion timestamp; trigger admission remains bound to the declared trigger-start window and completion remains bounded by the per-cycle timeout. The current row names its exact source schema version. |
| `COMPRESSED_PREDICTION_PLAN_V2.cycles[].predecessor` | Plan schema `2.2.0`: external predecessors carry exact plan SHA-256, collection prefix, manifest ID/hash, and mode-receipt ID/hash; current-plan predecessors derive those identifiers from the current plan. Missing or drifted evidence permits only one immutable DENIED ramp receipt and no ScanRun/event/manifest/mode write. |
...
| `CohortHeadroomReceipt` | Receipt ID includes the canonical c1-c5 input snapshot hash. Identical evidence reuses exact bytes; changed evidence creates another immutable receipt. Only a persisted exact PASS wire for the freshly recomputed snapshot may authorize c6, and the c6 manifest must name it in `headroom_receipt_id` and `input_artifact_ids`; DENIED writes no c6 ScanRun, event, manifest, or mode receipt. |
...
| `CohortDayManifest.parity` | Expected/actual newly-created and reused run counts, `new_epoch_required`, `same_write_path_as_ramp`, and `parity_match`. A valid ramp or reassessment requires all predicted runs newly created and zero reused; any partial-prefix recovery is `INCOMPLETE` and cannot gate a successor. |
...
| External `PrivacyReceiptSource` lock | A FULL_AUDIT_V1 preparation bundle uses wire `2.1.0` and binds the external receipt file SHA-256 plus the configured verifier-key fingerprint. Every receipt signature is verified before preparation; a missing row, changed source, changed verifier, or invalid signature fails closed. Legacy bundle `2.0.0` remains readable but cannot start FULL_AUDIT_V1. Secret key material is never written to the bundle. |
