...
| FULL_AUDIT PrivacyReceipt execution path | Admission accepts exactly either `LAB_LOCAL + LOCAL_PROCESS + OLLAMA_LOCAL` or the owner-approved private model leg `LAB_LOCAL + PRIVATE_SERVICE + OLLAMA_CLOUD_RUN`. Mixed triples and unlisted endpoints fail closed; `gemma.invoked=true` and `gemma.schema_valid=true` remain mandatory. |
