...
| WatchCase scheduler and lease | Active due case creates one bounded run | Future cases excluded; identical trigger repeated; injected crash before manifest | Hash-bound ScanRun/RUN_CREATED plus exact Firestore read-back equality | Repeated trigger creates no new run/event; crash retry reconciles authoritative runs and exactly one manifest | Day-1 manual selection/idempotency verified on LIVE infrastructure with SYNTHETIC data; date-isolated Day-N crash/resume is locally verified; managed Day-2 runtime remains unverified |
