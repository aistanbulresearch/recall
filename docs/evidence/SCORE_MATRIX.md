...
| Operational utility | Innovation 40% | Historical evidence signal reduces the cases a specialist must manually reopen | `docs/evaluation/EVALUATION_PROTOCOLS.md`; `docs/evaluation/HISTORICAL_REPLAY_CASE.md`; source manifest | planned | Positive and two controls frozen; product route and workload metric not executed |
...
| Memory authority and poisoning recovery | Architecture 30%, Fleet | Poisoned or contradictory Memory Bank entry is rejected; Firestore state and policy remain unchanged | `docs/security/THREAT_MODEL.md`; `docs/evaluation/EVALUATION_PROTOCOLS.md`; execution TBD | planned | Design fixed; Memory access and gate absent |
...
| Data-mode authenticity | Demo 30% | Every case, artifact, API response, and screen visibly distinguishes synthetic, replay, live public, and mock data | `docs/demo/WEB_INFORMATION_ARCHITECTURE.md`; execution TBD | planned | Presentation contract fixed; schemas and UI absent |
