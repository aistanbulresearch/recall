...
| WatchCase scheduler and lease | Active due case creates one bounded run | Future cases excluded; identical manual trigger repeated | Hash-bound ScanRun/RUN_CREATED plus exact Firestore read-back equality | Repeated trigger creates no new run/event; future cases create none | Day-1 selection/idempotency verified on LIVE infrastructure with SYNTHETIC data; lease/crash/resume still planned |
