...
def test_mints_bearer_header_from_gcloud(monkeypatch) -> None:
    ...
    header = provider.access_token_header()
...
def test_every_call_mints_fresh(monkeypatch) -> None:
    ...
    assert provider.access_token_header()["Authorization"] == "Bearer one"
    assert provider.access_token_header()["Authorization"] == "Bearer two"
...
def test_empty_token_is_an_error_not_an_empty_header(monkeypatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="gcloud_access_token_empty"):
        provider.access_token_header()
